...
class CompleteMe {
  ...
  suggest(partialWord) {
    ...
    if (currentBranch.isWord === true) {
      suggestion.push({value: partialWord.toLowerCase(), frequency: currentBranch.frequency, lastTouched: currentBranch.lastTouched});
    }
    ...
    const traveseTheTree = (partialWord, currentBranch) => {
      ...
      for (let i = 0; i < keys.length; i++) {
        ...
        const newString = partialWord.toLowerCase() + child.letter;
